...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `SLV💪 @${num.split('@')[0]}\nBem vindo ao grupo *${mdata.subject}* leia as regras pra nn ser removido mano`
				...
				teks = `Tchau Vai Nem Fazer Falta Man Kkkkkk ksksks@${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛*Rlx Ae Man Em Quanto Carrega Não Gostaria De Toma Uma Xicare De Café?*🤯⌛',
				success: '✔️Sucesso✔️',
				...
				only: {
					...
					ownerB: '[❗] Este comando só pode ser usado pelo Tody! ❌',
					admin: '[❗] Este comando só pode ser usado por administradores de grupo! ❌',
					Badmin: '[❗] Este comando só pode ser usado quando o bot se torna administrador! ❌',
                                        daftarB: `── 「REGISTRE-SE」 ──\nSLV mano como!\nVocê nn está registrado no banco de dados Do BOTZINMAKER tem que se cadastrar pra ter acesso ao menu, \n\nComando : ${prefix}daftar nome|idade\nExemplo : ${prefix}daftar bruxinho|19`,
				}
			}
